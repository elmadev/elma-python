...
def check_LGR_error(lgro,palette=None):
    ...
    if(type(lgro).__name__=="LGR"):
        ...
        for item in LGR_MANDATORY_FILES:
            ...
            for obj in lgro.images:
                if(item==obj.name.lower()):
                    ...
                    break
            if(n_count==0):
                message.append([ERR_LGR_MISSING_MANDATORY_FILE,item,"The LGR file is missing a mandatory file: %s"%(item)])
        ...
        if(n_tex<LGR_TEX_MIN):
            ...
            message.append([ERR_TOO_MANY_TEXTURES,n_tex,"The LGR file has %s texture image(s) but can only support %s at most"%(n_tex,LGR_TEX_MAX)])
        ...
        for i in range(9):
            if(apples[i]):
                if(appleFinished):
                    message.append([WARN_UNUSED_QFOOD,None,"Warning: qfood%s will not appear in the game as qfood%s is missing"%(i+1,i)])
            else:
                appleFinished=True
        ...
        try:
            if(len(bytes(lgro.padding))!=7):
                message.append([ERR_PADDING_INVALID,lgro,"%s's padding is invalid (must be an array of 7 ints in range (0-255))"%(lgro.name)])
        ...
        if(lgro.is_in_pictures_lst()):
                if(lgro.is_qup_qdown()):
                    if(lgro.img.height<LGR_WARNING_GRASS_HEIGHT_MIN):
                        message.append([WARN_GRASS_HEIGHT_TOO_SMALL,lgro,"Warning: %s should have a minimum height of %s, but the height is %s"%(lgro.name,LGR_WARNING_GRASS_HEIGHT_MIN,lgro.img.height)])
                elif((namelower in LGR_OBJECT_NAME) and namelower[:5]=="qfood"):
                    if(lgro.img.height!=LGR_OBJ_HEIGHT):
                        message.append([WARN_OBJ_HEIGHT_INVALID,lgro,"Warning: %s should have a height of %s, but the height is %s. If the height is smaller, the program might crash unexpectedly. If the height is larger, the additional columns will be ignored."%(lgro.name,LGR_OBJ_HEIGHT,lgro.img.height)])
                    if(lgro.img.width%LGR_OBJ_WIDTH!=0):
                        message.append([ERR_OBJ_WIDTH_INVALID,lgro,"As %s is an object, the width (%s) must be a multiple of %s"%(lgro.name,lgro.img.width,LGR_OBJ_WIDTH)])
                    ...
                    if(100>lgro.image_type<102):
                        message.append([ERR_IMAGE_TYPE_INVALID,lgro,"%s's image_type is invalid"%(lgro.name)])
                    else:
                        if(lgro.image_type==LGR_Image.PICTURE):
                            if(lgro.img.width>LGR_PIC_WIDTH_MAX):
                                message.append([ERR_PIC_TOO_WIDE,lgro,"%s is too wide (%s) - the maximum is %s"%(lgro.name,lgro.img.width,LGR_PIC_WIDTH_MAX)])
                            if(lgro.img.width*lgro.img.height>LGR_PIC_SIZE_MAX):
                                message.append([ERR_PIC_TOO_MANY_PIXELS,lgro,"%s has too many pixels (%s) - the maximum is %s. Depending on the image, the game might crash"%(lgro.name,lgro.img.width*lgro.img.height,LGR_PIC_SIZE_MAX)])
                        elif(lgro.image_type==LGR_Image.MASK):
                            is_mask=True
                    if(not(1<=lgro.default_distance<=999) and not(is_mask)):
                        message.append([ERR_DISTANCE_INVALID,lgro,"%s's distance (%s) is invalid (must be integer between 1-999)"%(lgro.name,lgro.default_distance)])
                    if(not(0<=lgro.default_clipping<=2) and not(is_mask)):
                        message.append([ERR_CLIPPING_INVALID,lgro,"%s's clipping (%s) is invalid"%(lgro.name,lgro.default_clipping)])
                    if(not(11<=lgro.transparency<=15)):
                        message.append([ERR_TRANSPARENCY_INVALID,lgro,"%s's transparency (%s) is invalid"%(lgro.name,lgro.transparency)])
        ...
        if(lgro.img):
            ...
            if((namelower in LGR_LIMITED_SIZE_FILES) and ((lgro.img.width>255)or(lgro.img.height>255))):
                message.append([ERR_SMALL_IMAGE_TOO_LARGE,lgro,"%s must have dimensions at most 255x255, but the dimensions are %sx%s"%(lgro.name,lgro.img.width,lgro.img.height)])
            if(lgro.is_valid_palette_image()):
                ...
                message.append([ERR_IMAGE_INVALID_PALETTE,lgro,"%s has an invalid palette"%(lgro.name)])
            if(namelower=="qcolors"):
                if(lgro.img.width!=LGR_WARNING_QCOLORS_WIDTH or lgro.img.height!=LGR_WARNING_QCOLORS_HEIGHT):
                    message.append([WARN_QCOLORS_WRONG_SIZE,lgro,"Warning: qcolors usually has dimensions of %sx%s, but the dimensions are %sx%s"%(LGR_WARNING_QCOLORS_WIDTH,LGR_WARNING_QCOLORS_HEIGHT,lgro.img.width,lgro.img.height)])
            if(namelower=="q1bike" or namelower=="q2bike"):
                if(lgro.img.width<LGR_RECOMMEND_BIKE_WIDTH or lgro.img.height<LGR_RECOMMEND_BIKE_HEIGHT):
                    message.append([WARN_QBIKE_TOO_SMALL,lgro,"Warning: %s needs dimensions of at least %sx%s to be rendered properly, but the dimensions are %sx%s"%(lgro.name,LGR_RECOMMEND_BIKE_WIDTH,LGR_RECOMMEND_BIKE_HEIGHT,lgro.img.width,lgro.img.height)])
        else:
