...
class TestLGR(unittest.TestCase):
    def test_LGR_Image(self):
        ...
        lgr1.palette[0]=300
        self.assertEqual(check_LGR_error(lgr1)[0][0],elma.error.ERR_LGR_INVALID_PALETTE)
        lgr1.palette[0]=-10
        self.assertEqual(check_LGR_error(lgr1)[0][0],elma.error.ERR_LGR_INVALID_PALETTE)
        lgr1.palette[0]=0
        ...
        self.assertEqual(check_LGR_error(lgr1)[0][0],elma.error.ERR_LGR_MISSING_MANDATORY_FILE)
        #ERR_DUPLICATE_NAME
        lgr1.images.extend([tempLGRIm,tempLGRIm])
        self.assertEqual(check_LGR_error(lgr1)[0][0],elma.error.ERR_DUPLICATE_NAME)
        lgr1.images.insert(0,tempLGRIm) #reset to normal
        lgr1.images.pop()
        lgr1.images.pop()
        #ERR_TOO_MANY_GRASS
        for i in range(30,80):  #skip the already-present qups and qdowns
            imgt=Image.new("P",(1,41),0)    #keep height of grass to avoid warning
            imgt.putpalette(lgr1.palette)
            lgr1.images.append(LGR_Image("qUp_%s"%(i),imgt,LGR_Image.PICTURE,999,LGR_Image.CLIPPING_G,LGR_Image.TRANSPARENCY_PAL_ZERO))
        for i in range(30,80):
            imgt=Image.new("P",(1,41),0)
            imgt.putpalette(lgr1.palette)
            lgr1.images.append(LGR_Image("qDowN_%s"%(i),imgt,LGR_Image.PICTURE,999,LGR_Image.CLIPPING_G,LGR_Image.TRANSPARENCY_PAL_ZERO))
        self.assertEqual(check_LGR_error(lgr1)[0][0],elma.error.ERR_TOO_MANY_GRASS)
        del lgr1.images[-100:]
        self.assertEqual(len(lgr1.images),len(lgr2.images))  #Just double-check we are back to the normal list
        #ERR_TOO_MANY_PICTURES
        for i in range(1000):
            imgt=Image.new("P",(1,1),0)
            imgt.putpalette(lgr1.palette)
            lgr1.images.append(LGR_Image("pic%s"%(i),imgt,LGR_Image.PICTURE,999,LGR_Image.CLIPPING_G,LGR_Image.TRANSPARENCY_PAL_ZERO))
        self.assertEqual(check_LGR_error(lgr1)[0][0],elma.error.ERR_TOO_MANY_PICTURES)
        ...
        for i in range(100):
            imgt=Image.new("P",(1,1),0)
            imgt.putpalette(lgr1.palette)
            lgr1.images.append(LGR_Image("pic%s"%(i),imgt,LGR_Image.TEXTURE,999,LGR_Image.CLIPPING_G,LGR_Image.TRANSPARENCY_PAL_ZERO))
        self.assertEqual(check_LGR_error(lgr1)[0][0],elma.error.ERR_TOO_MANY_TEXTURES)
        del lgr1.images[-100:]
        self.assertEqual(len(lgr1.images),len(lgr2.images))
        ...
        self.assertEqual(check_LGR_error(lgr1)[0][0],elma.error.ERR_NOT_ENOUGH_TEXTURES)
        ...
        for i in range(200):
            imgt=Image.new("P",(1,1),0)
            imgt.putpalette(lgr1.palette)
            lgr1.images.append(LGR_Image("pic%s"%(i),imgt,LGR_Image.MASK,999,LGR_Image.CLIPPING_G,LGR_Image.TRANSPARENCY_PAL_ZERO))
        self.assertEqual(check_LGR_error(lgr1)[0][0],elma.error.ERR_TOO_MANY_MASKS)
        del lgr1.images[-200:]
        self.assertEqual(len(lgr1.images),len(lgr2.images))
        #WARN_UNUSED_QFOOD
        ...
        self.assertEqual(check_LGR_error(lgr1)[0][0],elma.error.WARN_UNUSED_QFOOD)
        #ERR_NAME_TOO_LONG
        lgr1.images[-1].name="nameTOOlong"
        self.assertEqual(check_LGR_error(lgr1)[0][0],elma.error.ERR_NAME_TOO_LONG)
        #ERR_NAME_MISSING
        lgr1.images[-1].name=""
        self.assertEqual(check_LGR_error(lgr1)[0][0],elma.error.ERR_NAME_MISSING)
        #ERR_PADDING_INVALID
        lgr1.images[-1].name="x"
        lgr1.images[-1].padding=[1,2,3,4,5,6,7,8]
        self.assertEqual(check_LGR_error(lgr1)[0][0],elma.error.ERR_PADDING_INVALID)
        ...
        self.assertEqual(check_LGR_error(lgr1)[0][0],elma.error.WARN_GRASS_HEIGHT_TOO_SMALL)
        #ERR_OBJ_WIDTH_INVALID
        lgr1.images[-1].name="qFOOd3"
        self.assertEqual(check_LGR_error(lgr1)[0][0],elma.error.ERR_OBJ_WIDTH_INVALID)
        #WARN_OBJ_HEIGHT_INVALID
        imgt=Image.new("P",(40,1),0)
        imgt.putpalette(lgr1.palette)
        lgr1.images[-1].img=imgt
        self.assertEqual(check_LGR_error(lgr1)[0][0],elma.error.WARN_OBJ_HEIGHT_INVALID)
        #ERR_OBJ_TOO_WIDE
        imgt=Image.new("P",(40040,40),0)
        imgt.putpalette(lgr1.palette)
        lgr1.images[-1].img=imgt
        self.assertEqual(check_LGR_error(lgr1)[0][0],elma.error.ERR_OBJ_TOO_WIDE)
        ...
        self.assertEqual(check_LGR_error(lgr1)[0][0],elma.error.ERR_IMAGE_TYPE_INVALID)
        lgr1.images[-1].image_type=LGR_Image.PICTURE
        #ERR_PIC_TOO_WIDE
        imgt=Image.new("P",(6001,1),0)
        imgt.putpalette(lgr1.palette)
        lgr1.images[-1].img=imgt
        self.assertEqual(check_LGR_error(lgr1)[0][0],elma.error.ERR_PIC_TOO_WIDE)
        #ERR_PIC_TOO_MANY_PIXELS
        imgt=Image.new("P",(800,780),0)
        imgt.putpalette(lgr1.palette)
        lgr1.images[-1].img=imgt
        self.assertEqual(check_LGR_error(lgr1)[0][0],elma.error.ERR_PIC_TOO_MANY_PIXELS)
        #ERR_DISTANCE_INVALID
        imgt=Image.new("P",(1,1),0)
        imgt.putpalette(lgr1.palette)
        lgr1.images[-1].img=imgt
        lgr1.images[-1].default_distance=0
        self.assertEqual(check_LGR_error(lgr1)[0][0],elma.error.ERR_DISTANCE_INVALID)
        ...
        self.assertEqual(check_LGR_error(lgr1)[0][0],elma.error.ERR_CLIPPING_INVALID)
        lgr1.images[-1].default_clipping=-1
        self.assertEqual(check_LGR_error(lgr1)[0][0],elma.error.ERR_CLIPPING_INVALID)
        lgr1.images[-1].default_clipping=LGR_Image.default_clipping=LGR_Image.CLIPPING_G
        #ERR_TRANSPARENCY_INVALID
        lgr1.images[-1].transparency=100
        self.assertEqual(check_LGR_error(lgr1)[0][0],elma.error.ERR_TRANSPARENCY_INVALID)
        lgr1.images[-1].transparency=-1
        self.assertEqual(check_LGR_error(lgr1)[0][0],elma.error.ERR_TRANSPARENCY_INVALID)
        lgr1.images[-1].transparency=LGR_Image.TRANSPARENCY_PAL_ZERO
        #ERR_FILE_TOO_LARGE
        sizing=2400
        imgt=Image.new("P",(sizing,sizing),200)    #palette numbers 193-255 take up 2 bytes in .pcx format instead of one. 2400*2400*2 > 10,000,000
        imgt.putpalette(lgr1.palette)
        draw=ImageDraw.Draw(imgt)
        for i in range(0,sizing,2): #hatch the image because .pcx format uses run-length-encoding of same value in horizontal sequence
        ...
        self.assertEqual(check_LGR_error(lgr1)[0][0],elma.error.ERR_SMALL_IMAGE_TOO_LARGE)
        ...
        self.assertEqual(check_LGR_error(lgr1)[0][0],elma.error.WARN_PALETTE_MISMATCH)
        editted_palette[0]=0
        lgr1.images[4].put_palette(editted_palette)
        #ERR_IMAGE_INVALID_PALETTE
        imgt=Image.new("RGB",(1,1),0)
        lgr1.images.append(LGR_Image("x",imgt,LGR_Image.MASK,999,LGR_Image.CLIPPING_G,LGR_Image.TRANSPARENCY_PAL_ZERO))
        self.assertEqual(check_LGR_error(lgr1)[0][0],elma.error.ERR_IMAGE_INVALID_PALETTE)
        del lgr1.images[-1]
        ...
        self.assertEqual(check_LGR_error(lgr1)[0][0],elma.error.WARN_QCOLORS_WRONG_SIZE)
        ...
        imgt.putpalette(lgr1.palette)
        lgr1.images[index].img=imgt
        self.assertEqual(check_LGR_error(lgr1)[0][0],elma.error.WARN_QBIKE_TOO_SMALL)
